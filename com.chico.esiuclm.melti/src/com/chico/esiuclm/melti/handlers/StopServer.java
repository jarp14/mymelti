	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		if(Proxy.get().getJettyServer().getState().equals("STARTED")) {
			try {
				...
				MessageDialog.openInformation(window.getShell(), "Jetty Server", "Escucha pausada con éxito");
				Display.getDefault().syncExec(new Runnable() {
					public void run() {
						my_console.setColor(new Color(null, new RGB(0,204,0)));
						my_console.println("["+new Date().toString()+"] OK: La escucha del servidor Jetty ha sido pausada con éxito");
					}
				});
			} catch (Exception e) {
				e.printStackTrace();
			...
			MessageDialog.openInformation(window.getShell(), "Jetty Server", "La escucha ya se encuentra pausada");
			Display.getDefault().syncExec(new Runnable() {
				public void run() {
					my_console.setColor(new Color(null, new RGB(204,204,0)));
					my_console.println("["+new Date().toString()+"] WARNING: La escucha del servidor Jetty ya se encuentra pausada");
				}
			});
		}
